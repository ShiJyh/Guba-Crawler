...
class guba_comments:
    ...
    def __init__(
        ...
        # 备用代理配置
        self.backup_proxies = {
            "http": "http://%(user)s:%(pwd)s@x292.kdltps.com:15818/" % {"user": username, "pwd": password},
            "https": "http://%(user)s:%(pwd)s@x292.kdltps.com:15818/" % {"user": username, "pwd": password}
        }
    ...
    def get_soup_form_url(self, url: str) -> BeautifulSoup:
        ...
        response = requests.get(
            ...
            timeout=10,
            proxies=self.backup_proxies if self.use_backup_proxy else self.proxies
        )
